...
- **Supabase-Deploy DONE (Claude):** `supabase db push --linked` (Migrationen 0008 Briefing-Archiv + 0009 user_settings jetzt auf der Remote-DB) + `npm run deploy` (Worker `8f3cf6dc`, mcp.katoos.de). Verifiziert: GET/PUT /api/me/settings -> 401 ohne Token, OPTIONS 204, /health 200. Damit auch Briefing-Archiv-Persistenz + Dokument-Endpunkte + Tenant-Guard endlich live.
- **Cloud-Profil App-Seite (Zero-Knowledge) gebaut + getestet:** Rust-Krypto Argon2id (Parameter GEPINNT, nicht Crate-Default) + AES-256-GCM, Schluessel NUR im RAM (zeroize beim Logout). Commands cloud_profile_sync_after_login/push/unlock_and_push/logout. Login holt+entschluesselt das Profil (oder legt es an); Speichern pusht; still-wieder-eingeloggt -> einmalige Passwort-Abfrage. Sichtbarer Logout in der Sidebar-Fusszeile + Dialog (Bestaetigen -> Spinner "Cloud-Profil wird gesichert" -> ggf. Passwort -> ggf. Fehler mit "Trotzdem abmelden"). Logout = ERST sichern, DANN tenant-sauber raeumen (Keychain/Config-Felder/localStorage-Caches/RAM-Key/Session) -> LoginGate-Lockdown. i18n De/En/Es/Ru.
- **Adversariale Review (12 Agenten, 4 Dim x Verify): 8 Funde, ALLE gefixt:** Argon2-Parameter pinnen (sonst macht ein Crate-Update Profile still unlesbar); zeroize fuer Schluessel/Blob; Salt-Robustheit (korruptes Salt heilt sich statt zu blockieren); Logout leert auch Codex-Feed/Status/Tageszaehler/Logs/Queue-State + runHistory (Cross-Tenant-UI-Leck); board.completed-Key + loginEmail geraeumt; setSessionStatus robust nach boot() (kein Aussperren bei boot-Fehler); Passwort-Klartext ueberlebt nicht im Dialog-State (Mounts gegatet).
- **Live-Test-Funde (vom Nutzer) gefixt:** (1) base_url-Bug -- die gespeicherte baseUrl war `https://mcp.katoos.de/mcp`, der /mcp-Pfad brach ALLE /api-Aufrufe (404 "Route not found"; traf still auch Action-Plaene/Briefings). Fix: normalize_base_url liefert jetzt IMMER nur scheme://host (extract_allowed_host, userinfo/port-sicher); normalize_config heilt die gespeicherte base_url. (2) Falsch-Passwort-Bug -- ohne vorhandenes Profil wurde JEDES Passwort akzeptiert und das Profil unter falschem Schluessel angelegt. Fix: verify_account_password (GoTrue-Re-Login) verifiziert das Passwort gegen das Konto, wenn kein entschluesselbares Profil zum Gegenpruefen da ist. Nachpruefung beider Fixes (2 Probe-Agenten) sauber.
- **UX-Funde (vom Nutzer) gefixt:** Setup-Karte zeigt alle 5 Schritte mit Erledigt/Fehlt (fehlende orange) statt nur Prozent; manueller Connector-Token-Weg explizit ("Uebernehmen"-Button + Hinweis) -> vorhandenen Token bestaetigen ohne Rotations-Zwang (Mistral-Connector bleibt gueltig).
- **OFFEN:** Commit pushen (Nutzer-OK abwarten); Supabase-Auth-Config #1/#2/#7a (Site URL weg von localhost / Confirm-Email / Passwort-Staerke -- Management-API-Token oder Dashboard); beta.7 Release; Welle-C-Review (KatoContext); Datei-Modus + Claude-Runner Live-Test. Build/Sign-Ablauf: signierte Dev-Builds (Developer ID) -> API-Key-Persistenz; Release zusaetzlich notarisieren.
